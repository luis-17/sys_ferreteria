...
class Model_aviso extends CI_Model {
	...
	public function m_cargar_avisos_intranet()
	{
		$this->db->select('idaviso, idaviso,idsedeempresaadmin,iduser,idempleado,titulo,redaccion,fecha_creacion,estado_av');
		$this->db->from('intr_aviso');
		$this->db->where('estado_av', 1);
		$this->db->order_by('fecha_creacion', 'DESC');
		$this->db->limit(10);
		return $this->db->get()->result_array();
	}
	public function m_cargar_avisos($paramPaginate){
		$this->db->select("(nombres || ' ' || apellido_paterno || ' ' || apellido_materno) AS empleado", FALSE);
		$this->db->select('idaviso, idaviso,idsedeempresaadmin,emp.idempleado,titulo,redaccion,fecha_creacion,estado_av, ');

...
<div class="modal-body">
    <form class="row" name="formSolicitud" id="formSolicitud"> 
		...
		<div class="form-group col-md-6 mb-md">
			<label class="control-label mb-xs"> Procedimiento <small class="text-danger">(*)</small></label>
			<div class="input-group">
				<span class="input-group-btn ">
					<input type="text" class="form-control input-sm" style="width:45px;text-align:right;margin-right:4px;" ng-model="fData.idproductomaster" placeholder="ID" tabindex="103" ng-enter="obtenerProcedimientoPorCodigo(); $event.preventDefault();" ng-readonly="true" required="true"/>
				</span>
				<input id="fDataProcedimiento" type="text" class="form-control input-sm" ng-model="fData.procedimiento" placeholder="Ingrese el Procedimiento" typeahead-loading="loadingProcedimiento" uib-typeahead="item as item.descripcion for item in getProcedimientoAutocomplete($viewValue)" typeahead-on-select="getSelectedProcedimiento($item, $model, $label)" typeahead-min-length="2" tabindex="104" ng-change="fData.idproductomaster = null; noResultsLP = false"/>
			</div>
			<i ng-show="loadingProcedimiento" class="fa fa-refresh"></i>
		...
		<div class="form-group col-md-12 mb-md">
			...
			<textarea class="form-control input-sm" ng-model="fData.observacion" tabindex="105"></textarea>
		</div>
	</form>
</div>

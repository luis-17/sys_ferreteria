angular.module('theme.documentoInterno', ['theme.core.services'])
  .controller('documentoInternoController', ['$scope', '$sce', '$modal', '$bootbox', '$window', '$theme', 'uiGridConstants', 'pinesNotifications', 'hotkeys'
    ...
    $scope.gridOptions = {
      ...
      multiSelect: true,
      columnDefs: [
        { field: 'id', name: 'idaviso', displayName: 'ID', width: '4%' },
    ...
    $scope.getPaginationServerSide = function() {
      ...
      documentoInternoServices.sListarAvisos(arrParams).then(function (rpta) {
    ...
    $scope.btnEditar = function (size) {
      $modal.open({
        templateUrl: angular.patchURLCI+'DocumentoInterno/ver_popup_formulario',
        size: size || '',
        backdrop: 'static',
        keyboard:false,
        scope: $scope,
        controller: function ($scope, $modalInstance) {
          ...
          $scope.aceptar = function () {
            documentoInternoServices.sEditar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $modalInstance.dismiss('cancel');
    ...
    $scope.btnNuevo = function (size) {
      $modal.open({
        ...
        controller: function ($scope, $modalInstance) {
          ...
          $scope.aceptar = function () {
            documentoInternoServices.sRegistrar($scope.fData).then(function (rpta) {
              if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $modalInstance.dismiss('cancel');
     ...
     $scope.btnAnular = function (mensaje) { 
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          documentoInternoServices.sAnular($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
   ...
   $scope.btnHabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          documentoInternoServices.sHabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
    ...
    $scope.btnDeshabilitar = function (mensaje) {
      ...
      $bootbox.confirm(pMensaje, function(result) {
        if(result){
          documentoInternoServices.sDeshabilitar($scope.mySelectionGrid).then(function (rpta) {
            if(rpta.flag == 1){
                pTitle = 'OK!';
                pType = 'success';
                $scope.getPaginationServerSide();
    ...
    hotkeys.bindTo($scope)
      .add({
        combo: 'alt+n',
        description: 'Nuevo Documento Interno',
        callback: function() {
          $scope.btnNuevo();
        }
      })
      .add ({
        combo: 'e',
        description: 'Editar Documento Interno',
        callback: function() {
          if( $scope.mySelectionGrid.length == 1 ){
            $scope.btnEditar();
          }
      ...
      .add ({
        combo: 'del',
        description: 'Anular Documento Interno',
        callback: function() {
      ...
      .add ({
        combo: 'b',
        description: 'Buscar Documento Interno',
        callback: function() {
          $scope.btnToggleFiltering();
        }
      });
  }])
  .service("documentoInternoServices",function($http, $q) {
    return({
      sListarDocumentosInternoIntranet: sListarDocumentosInternoIntranet
    });
    function sListarDocumentosInternoIntranet (datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"DocumentoInterno/lista_documentos_internos_intranet",
            data : datos
      });

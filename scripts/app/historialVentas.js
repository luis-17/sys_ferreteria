angular.module('theme.historialVentas', ['theme.core.services'])
  .controller('historialVentasController', ['$scope', '$filter', '$route', '$sce', '$interval', '$modal', '$bootbox', '$window', '$http', '$theme', '$log', '$timeout', 'uiGridConstants', 'pinesNotifications', 'hotkeys', 
    'cajaActualServices', 
    'empresaAdminServices',
    'especialidadServices', 
    'historialVentaServices',
    'convenioServices',
    ...
    empresaAdminServices.sListarSedeEmpresaAdminCbo().then(function (rpta) { //console.log(rpta);
      ...
      $scope.fBusquedaWeb.sedeempresa = $scope.listaSedeEmpresaAdmin[ind];
    ...
    $scope.gridOptions = { 
      ...
      columnDefs: [ 
        ...
        { field: 'tipodocumento', name: 'descripcion_td', displayName: 'Tipo Doc.', width: '9%', cellClass: 'bg-lightblue', sort: { direction: uiGridConstants.DESC} },
    ...
    $scope.getPaginationServerSide = function() {
      ...
      historialVentaServices.sListarVentasHistorial(arrParams).then(function (rpta) { 
      ...
      $modal.open({
        ...
        controller: function ($scope, $modalInstance) { 
          ...
          $scope.aceptar = function(){
            historialVentaServices.sImprimirTicketVentaManual($scope.fVenta).then(function (rpta) { 
              if(rpta.flag == 1){
                var printContents = rpta.html;
                var popupWin = window.open('', 'windowName', 'width=300,height=300');
                popupWin.document.open()
    ...
    $scope.fBusquedaWeb.desde = $filter('date')(new Date(),'dd-MM-yyyy');
    $scope.fBusquedaWeb.desdeHora = '00';
    $scope.fBusquedaWeb.desdeMinuto = '00';
    $scope.fBusquedaWeb.hastaHora = 23;
    $scope.fBusquedaWeb.hastaMinuto = 59;
    $scope.fBusquedaWeb.hasta = $filter('date')(new Date(),'dd-MM-yyyy');
    $scope.mySelectionGridVWeb = [];
    var paginationOptionsWeb = { 
      pageNumber: 1,
      firstRow: 0,
      pageSize: 10,
      sort: uiGridConstants.DESC,
    ...
    $scope.gridOptionsWeb = { 
      ...
      columnDefs: [ 
        ...
        { field: 'ticket', name: 'ticket_venta', displayName: 'Ticket', width: '9%', sort: { direction: uiGridConstants.DESC} },
        { field: 'fecha_venta', name: 'fecha_venta', displayName: 'Fecha de Venta', width: '12%', enableFiltering: false  },
        { field: 'medio', name: 'descripcion_med', displayName: 'Medio de Pago', width: '10%', visible: true },
        { field: 'subtotal', name: 'sub_total', displayName: 'SubTotal', width: '7%' },
        { field: 'igv', name: 'total_igv', displayName: 'I.G.V.', width: '7%' },
        { field: 'total', name: 'total_a_pagar', displayName: 'Total', width: '7%', cellClass: 'bg-lightblue' }, 
        { field: 'monto_comision', name: 'monto_comision', displayName: 'COMISIÓN', width: '7%' }, 
        { field: 'estado', type: 'object', name: 'estado', displayName: ' ', width: '5%', enableFiltering: false, enableSorting: false, enableColumnMenus: false, enableColumnMenu: false, 
          cellTemplate:'<div class="">'+ 
            '<label tooltip-placement="left" tooltip="{{ COL_FIELD.labelText }}" style="box-shadow: 1px 1px 0 black; margin: 6px auto; min-width: 18px;" class="label {{ COL_FIELD.claseLabel }} ml-xs">'+ 
            '<i class="fa {{ COL_FIELD.claseIcon }}"></i> </label>'+ 
      ...
      onRegisterApi: function(gridApiWeb) { 
        $scope.gridApiWeb = gridApiWeb;
        gridApiWeb.selection.on.rowSelectionChanged($scope,function(row){ 
          $scope.mySelectionGridVWeb = gridApiWeb.selection.getSelectedRows();
        });
        gridApiWeb.selection.on.rowSelectionChangedBatch($scope,function(rows){
          $scope.mySelectionGridVWeb = gridApiWeb.selection.getSelectedRows();
        });

        $scope.gridApiWeb.core.on.sortChanged($scope, function(grid, sortColumns) { 
          //console.log(sortColumns);
          if (sortColumns.length == 0) {
            paginationOptionsWeb.sort = null;
            paginationOptionsWeb.sortName = null;
          } else {
            paginationOptionsWeb.sort = sortColumns[0].sort.direction;
            paginationOptionsWeb.sortName = sortColumns[0].name;
          }
          $scope.getPaginationServerSideWeb();
        });
        $scope.gridApiWeb.core.on.filterChanged( $scope, function(grid, searchColumns) { 
          var grid = this.grid;
          paginationOptionsWeb.search = true; 
          paginationOptionsWeb.searchColumn = { 
            'orden_venta' : grid.columns[1].filters[0].term,
            "CONCAT(c.nombres,' ',c.apellido_paterno,' ',c.apellido_materno)" : grid.columns[2].filters[0].term,
            'descripcion_td' : grid.columns[3].filters[0].term,
            'ticket_venta' : grid.columns[4].filters[0].term,
        ...
        gridApiWeb.pagination.on.paginationChanged($scope, function (newPage, pageSize) {
          paginationOptionsWeb.pageNumber = newPage;
          paginationOptionsWeb.pageSize = pageSize;
          paginationOptionsWeb.firstRow = (paginationOptionsWeb.pageNumber - 1) * paginationOptionsWeb.pageSize;
          $scope.getPaginationServerSideWeb();
        });
      }
    };
    paginationOptionsWeb.sortName = $scope.gridOptionsWeb.columnDefs[3].name;
    $scope.getPaginationServerSideWeb = function() { 
      var arrParams = {
        paginate : paginationOptionsWeb,
        datos : $scope.fBusquedaWeb
      };
      historialVentaServices.sListarVentasWebHistorial(arrParams).then(function (rpta) { 
        $scope.gridOptionsWeb.totalItems = rpta.paginate.totalRows;
        $scope.gridOptionsWeb.totalImporte = rpta.paginate.sumTotal;
        $scope.gridOptionsWeb.data = rpta.datos;
      });
      $scope.mySelectionGridVWeb = [];
    };
  }])
  .service("historialVentaServices",function($http, $q) {
    return({
        sListarVentasHistorial: sListarVentasHistorial,
        sListarVentasWebHistorial:sListarVentasWebHistorial,
    ...
    function sListarVentasHistorial(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"historialVentas/lista_historial_ventas", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sListarVentasWebHistorial(datos) {
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"historialVentas/lista_historial_ventas_web", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    }
    function sImprimirTicketVentaManual(datos) { 
      var request = $http({
            method : "post",
            url : angular.patchURLCI+"venta/imprimir_ticket_venta_manual", 
            data : datos
      });
      return (request.then( handleSuccess,handleError ));
    } 
